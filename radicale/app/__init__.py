...
class Application(
    ...
    def __init__(self, configuration: config.Configuration) -> None:
        ...
        self._permit_overwrite_collection = configuration.get(
            "rights", "permit_overwrite_collection"
        )
        logger.info(
            "permit overwrite of collection: %s", self._permit_overwrite_collection
        )
    ...
    def _handle_request(self, environ: types.WSGIEnviron) -> _IntermediateResponse:
            ...
            if answer is not None:
                if isinstance(answer, str):
                    if self._response_content_on_debug:
                        ...
                        logger.debug(
                            "Response content: suppressed by config/option [logging] response_content_on_debug"
                        )
        ...
        if self._request_header_on_debug:
            ...
            logger.debug(
                "Request header: suppressed by config/option [logging] request_header_on_debug"
            )
